    ...
    Process
        {
            ...
            $R_entry.computername = $ComputerName
            $er = (Get-WmiObject  win32_computersystem -ComputerName $ComputerName -ErrorAction SilentlyContinue).name

            if (($er -ne $ComputerName) -and ($ComputerName -ne 'localhost'))
                {
                ...
                $R_entry.Total_RAM_mb = ((Get-WmiObject  win32_computersystem -ComputerName $ComputerName).totalphysicalmemory / 1mb -as [int])
                ...
                $memslots = $memorytype[(Get-WmiObject Win32_PhysicalMemory -ComputerName $ComputerName).memorytype ] | Group-Object
                foreach ($item in $memslots)
                    {
                    $memrep += [string]$item.count + " x " + [string]$item.Name + '   '
                    }
                $R_entry.Memory_slots = $memrep
                $memspace = (Get-WmiObject Win32_PhysicalMemory -ComputerName $ComputerName).capacity | Group-Object
                foreach ($item in $memspace)
                    {
                    $memcount += [string]$item.count + " x " + [string]($item.Name/ 1mb -as [int]) + ' MB  '
                    }
                $R_entry.Memory_installed = $memcount
                $R_entry.MB_Vendor = (Get-WmiObject Win32_BaseBoard -ComputerName $ComputerName).manufacturer
                $R_entry.MB_Model = (Get-WmiObject Win32_BaseBoard -ComputerName $ComputerName).product
                $R_entry.Processor = [string](Get-WmiObject win32_processor -ComputerName $ComputerName).name
                $R_entry.VideoAdapter = [string](Get-WmiObject Win32_VideoController -ComputerName $ComputerName).name
                $R_entry.Monitor = [string](Get-WmiObject Win32_Desktopmonitor -ComputerName $ComputerName).name
